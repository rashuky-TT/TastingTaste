...
def setMsgQ():
    global tcId
    global msgQ
    tcId = PythonController.i_SetTargetPitch
    msgQ = True
...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_egse_RI_SetTargetPitch
    tcId = i_egse_RI_SetTargetPitch
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global SetTargetPitch_via_shared_lib
    shared_lib = True
    SetTargetPitch_via_shared_lib = dll.egse_PI_SetTargetPitch
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.PITCH_VALUE_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def decode_ACN(ACN_encodedBuffer):
    ...
    decoded_value = ASN1.PITCH_VALUE()
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.PITCH_VALUE_REQUIRED_BYTES_FOR_ENCODING)
...
def sendTC(tc):
    ...
    if msgQ:
        if checkConstraints(tc):
            try:
                PythonController.Invoke_SetTargetPitch(tc)
            except IOError:
                raise
    elif udp:
