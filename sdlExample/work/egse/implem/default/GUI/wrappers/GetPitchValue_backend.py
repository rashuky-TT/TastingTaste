...
def setMsgQ():
    global tmId
    tmId = PythonController.i_GetPitchValue


def setUDP():
    global tmId
    from _InterfaceEnum import i_egse_PI_GetPitchValue
    tmId = i_egse_PI_GetPitchValue


def GetPitchValue(tm_ptr, size):
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.PITCH_SEQ_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.PITCH_SEQ_REQUIRED_BYTES_FOR_ENCODING)
...
def expect(Q, VNvalue, ignoreOther=False, timeout=None):
    ...
    while True:
        ...
        if msgId == tmId:
            #expectedValue = '{ GetPitchValue ' + VNvalue + ' }'
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received GetPitchValue with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected GetPitchValue (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for GetPitchValue') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type PITCH_SEQ
    # GetPitchValue = ASN1.PITCH_SEQ()
    return None

def typeInstance():
    # Create an intance of an ASN.1 ctype
    return ASN1.PITCH_SEQ()
