...
package body Ks is
   ...
   procedure SetPitch(targetPitch: in out asn1SccPITCH_VALUE) is
      begin
         case ctxt.state is
            when asn1Scccontrol =>
               ctxt.targetpitch := targetPitch;
      ...
      begin
         while (trId /= -1) loop
            case trId is
               ...
               when 1 =>
                  --  NEXT_STATE Control (41,22) at 449, 120
                  trId := -1;
                  ctxt.State := asn1SccControl;
                  goto Continuous_Signals;
               when 2 =>
                  --  NEXT_STATE Control (48,22) at 502, 402
                  trId := -1;
                  ctxt.State := asn1SccControl;
                  goto Continuous_Signals;
               when 3 =>
                  ...
                  --  GetPitch(pitch) (54,19)
                  RI_0_GetPitch(ctxt.pitch);
                  --  NEXT_STATE Control (56,22) at 649, 512
                  trId := -1;
                  ctxt.State := asn1SccControl;
                  goto Continuous_Signals;
               when CS_Only =>
